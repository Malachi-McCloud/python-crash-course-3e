class Car:
    # A simple attempt to represent a car
    
    def __init__(self, make, model, year):
        # Initialize the attributes to our car
        self.make = make
        self.model = model
        self.year = year
        self.odometer_reading = 0

    def get_descriptive_name(self):
        # Return a neatly formatted and descriptive name
        long_name = f"{self.year} {self.make} {self.model}"
        return long_name.title()
    
    def read_odometer(self):
        # print a statement showing the cars mileage
        print(f"This car has a {self.odometer_reading} miles on it.")
    
    def update_odometer(self, mileage):
        # Set the odometer reading to the given value
        # Reject the change if it attempts to roll the odometer back
        
        if mileage >= self.odometer_reading:
            self.odometer_reading = mileage
        else:
            print("You can't roll back the odometer.")

    def increment_odometer(self, miles):
        # Add the given amount to the odometers reading
        self.odometer_reading += miles


# Instances as attributes 
class Battery:
    # A simple attempt to model a battery for an electric car.
    def __init__(self, battery_size=40):
        self.battery_size = battery_size

    def describe_battery(self):
        # a statement describing the size of the battery itself
        print(f"This car has a {self.battery_size}--kWh battery.")
        
     # get range 
    def get_range(self):
        # Print aa statement about the range this battery provides
        if self.battery_size == 40:
            range = 150
        elif self.battery_size == 65:
            range = 225

        print(f"This car can go about {range} miles on a full charge.")


class ElectricCar(Car):
    # Represents apsects of a car, specific to electric vehicles

    def __init__(self, make, model, year):
        # Initialize the attributes as before
        super().__init__(make, model, year)
        self.battery = Battery()

    # def describe_battery(self):
    #     # Print a statement describing the battery life
    #     print(f"This car has a {self.battery_size}--kWh battery.")
    
    def fill_gas_tank(self):
        # Electric Cars don't have a gas tank
        print("Electric cars don't have a gas tank!")






my_leaf = ElectricCar('nissan', 'leaf', 2024)
print(my_leaf.get_descriptive_name())
my_leaf.battery.describe_battery()
my_leaf.battery.get_range()
my_leaf.fill_gas_tank()

my_used_car = Car('ford', 'edge', 2010)
print(my_used_car.get_descriptive_name())

my_used_car.update_odometer(23_500)
my_used_car.read_odometer()


my_new_car = Car('toyota','camry', 2020)
print(my_new_car.get_descriptive_name())

my_new_car.odometer_reading = 101_000
print(my_new_car.read_odometer())

my_new_car.update_odometer(101_001)
print(my_new_car.read_odometer())


